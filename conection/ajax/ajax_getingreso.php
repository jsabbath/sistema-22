...
if (isset($_POST['corre_ingreso'])) {
    $corre_ingreso = $_POST['corre_ingreso'];

    require_once("../fun_sistema.php");
    $ad = new fun_sistema();
    $conexion = $ad->conectarBD();
    $consulta = "select * from vistapagos where correlativo=$corre_ingreso and tipo=2";
    ...
    if ($filas) {
        while ($rs = mysqli_fetch_array($registro)) {
            ...
            $_SESSION['in_adjunto_idingreso'] = $rs[0];
...
if (isset($_GET['corre_facturas'])) {
    ...
    $conexion = $ad->conectarBD();
    //Creamos la consulta
    $consulta = "call pagodetallefacturas($corre_facturas,2)";
...
if (isset($_GET['idpago_adjunto'])) {
    ...
    //Creamos la consulta
    $consulta = "select * from vistaarchivo where idpadre=$idpago_adjunto and (tabla_padre='in' and estado=1);";
    //obtenemos los registros de la consulta
    mysqli_set_charset($conexion, "utf8");
    $registro = mysqli_query($conexion, $consulta);
    //obtenemos el array con toda la información
