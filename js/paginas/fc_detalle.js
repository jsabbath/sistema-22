$(document).ready(function () {
    ...
    $('#upload').click(function () {
        ...
        $.ajax({
            dataType: "json",
            url: '../conection/ajax/subirArchivoFcompra.php',
            type: 'POST',
            data: formData,
            cache: false,
            contentType: false,
...
function calcular() {
    ...
    document.getElementById("fact_bruto").value = addCommas(parseFloat(varSubTotal).toFixed(0));
}
...
function imprimirFactura(r) {
    ...
    if (r.estado !== 0) {
        document.getElementById("id_fact").value = r.iddocumento;
        document.getElementById("fact_fecha_doc").value = r.fecha_doc;
        document.getElementById("fact_fecha_plazo").value = r.fecha_plazo;
        document.getElementById("fact_prove_nom").value = r.clieprove_nombre;
        document.getElementById("fact_neto").value = r.neto;
        document.getElementById("fact_iva").value = r.iva;
        document.getElementById("fact_total").value = r.total;
        document.getElementById("fact_glosa").value = r.glosa;
        if (r.estado == 1) {
            ...
            $("#btn_save").addClass("disabled");
            $("#btn_eli_fc").addClass("disabled");
            $("#btn_eli_fc").attr("onclick", "''");
            ...
            $.ajax({
                url: "../conection/ajax/ajax_getfactura.php?tipo_doc=1",
                type: "POST",
...
$("#boton_pago").click(function () {
    window.open("fc_egreso_detalle.php?corre_eg=" + correlativo_factura, '_blank');
});
function editarFc() {
    $("#btn_edit").addClass("disabled");
